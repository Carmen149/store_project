...
public class CustomerController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(code = 400, message = """
                                                ...
                                                """) })
    @PostMapping()
    public ResponseEntity<?> saveCustomer(@ApiParam(name = "Customer")@Valid @RequestBody CustomerDto customerDto) {
        try {
    ...
    public ResponseEntity<?>exportCustomerDetails(@PathVariable("id") Long id,@RequestParam String fileType)
    {
        return new ResponseEntity(customerService.exportCustomerDetails(id,fileType),HttpStatus.OK);
    }
    @PostMapping("/login")
    public ResponseEntity<Customer>logIn(@RequestBody AuthDTO authDTO){
        return new ResponseEntity<>(customerService.findByUserName(authDTO),HttpStatus.OK);
    }
}
