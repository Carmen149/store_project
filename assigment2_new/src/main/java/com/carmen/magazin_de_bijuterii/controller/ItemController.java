   ...
   // @SneakyThrows
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(code = 400, message =  """
                                                ...
                                                """) })
    @PostMapping
    public ResponseEntity<?> saveItem(@ApiParam(name = "Item")@RequestBody ItemDto itemDto){
        try{
            ...
            if (t instanceof ConstraintViolationException) {
                ...
                return new ResponseEntity<>(messages.stream().findFirst(), HttpStatus.BAD_REQUEST);
            }
            else{
...
//    })
    ...
    @GetMapping("{type}/{priceMin}/{priceMax}/{availability}/{size}/{material}/{list}")
    public List<Item>filterList(@PathVariable("type") Type type, @PathVariable("priceMin") Double priceMin, @PathVariable("priceMax")Double priceMax,@PathVariable("availability") int availability,@PathVariable("size") Integer size, @PathVariable("material") String material,@PathVariable("list") List<String>list){
        List<Item> items= itemService.getAllItems();
        if(availability==1){
            return itemService.filterList(type,priceMin,priceMax,true,size,material,list,items);
        }else{
            return itemService.filterList(type,priceMin,priceMax,false,size,material,list,items);
        }
    }
