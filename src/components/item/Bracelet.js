...
class Bracelet extends React.Component {
  ...
  componentDidMount(){
    ...
    axiosInstance.get("api/items/get/bracelet")
    .then(
      res => {
        const val = res.data;
        this.setState({
  ...
  add=(id)=>{
    ...
    let idCommand=0;
    console.log(this.state.bracelet)
  ...
  filter=()=>{
   ...
   console.log(list);
    let url=encodeURI(`api/items/BRACELET/${this.state.priceMin}/${this.state.priceMax}/${availability}/${this.state.size}/${this.state.material}/${ordonare}/${list}`)
    console.log(url)
...
.catch(error => {
  ...
  ordonare=()=>{
    if(this.state.asc===true){
      axiosInstance.post(`api/items/orderAsc/BRACELET`)
      .then(res => {
        const val = res.data;
        console.log(val);
        this.setState({
    ...
    if(this.state.desc===true){
      axiosInstance.post(`api/items/orderDesc/BRACELET`)
      .then(res => {
        const val = res.data;
        console.log(val);
        this.setState({
        ...
        <Container>
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 10fr)",gridGap: "20px"}}>
              {this.state.bracelets.map((bracelet,index)=>(   
                <Card key={index} style={{ width: '18rem' }} >
                  <Card.Img variant="top" src={this.state.images[bracelet.nameImg+'.jpg']}></Card.Img>
                  <Card.Body>
                    <Card.Title >{bracelet.name}</Card.Title><br></br>
                    <Card.Subtitle className="mb-2 text-muted">Pret: {bracelet.price}</Card.Subtitle>
                    <Card.Text>
                      Atunci cand, cuvintele nu sunt suficiente pentru a-ti exprima sentimentele, lasa bratarile Euphoria sa vorbeasca pentru tine. Indiferent de tinuta la ca accesorizezi bratarile Euphoria, cu siguranta te vei face remarcata.
                    </Card.Text>
                    <div>
                      <Button variant="dark"  onClick={()=>{this.viewDetails(bracelet.idItem,index)}}>Details</Button>{'  '}
                      <Button variant="dark" style={{marginLeft:'10px'}} onClick={()=>{this.add(bracelet.idItem)}}>Add to cart</Button>{'  '}
                      <Button  variant="dark"  style={{marginLeft:'10px'}}onClick={()=>{this.addMyList(bracelet.idItem,index)}}>< BsSuitHeart /></Button>   {'  '}   
                    </div>
                  </Card.Body>
                </Card>
              ))}
